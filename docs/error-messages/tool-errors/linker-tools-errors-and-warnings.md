...
リンク、LIB、DUMPBIN、および EDITBIN は、これらのエラーと警告を生成します。 エラー番号は、出力ウィンドウの上にマウス ポインターをクリックするか特定のエラー メッセージに関するヘルプを表示し、内のエラー番号を入力、F1 キーを押して、**探します**ボックスに、インデックス、または次のリンクをクリックします。  

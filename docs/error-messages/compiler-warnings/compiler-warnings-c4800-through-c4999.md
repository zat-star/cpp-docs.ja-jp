...
記事、ドキュメントのこの部分には、Visual C コンパイラの警告のサブセットに関する情報が含まれています。 ここでの情報にアクセスするまたは、Visual Studio で [出力] ウィンドウに、エラー番号を選択し、F1 キーを押します。  
  
## <a name="in-this-section"></a>このセクションの内容  
  
|警告|メッセージ|  
|-------------|-------------|  
|[コンパイラの警告 (レベル 3) C4800](../../error-messages/compiler-warnings/compiler-warning-level-3-c4800.md)|'type': 値を強制的に bool 'true' または 'false' (パフォーマンスの警告)|  
|[コンパイラの警告 (レベル 1) C4803](../../error-messages/compiler-warnings/compiler-warning-level-1-c4803.md)|'method': raise メソッド、イベントと異なるストレージ クラスには 'event'|  
|[コンパイラの警告 (レベル 1) C4804](../../error-messages/compiler-warnings/compiler-warning-level-1-c4804.md)|'操作': 'bool' 操作で型の安全でない使用|  
|[コンパイラの警告 (レベル 1) C4805](../../error-messages/compiler-warnings/compiler-warning-level-1-c4805.md)|'操作': 型 'type' のミックスの安全でないと型 'type' 操作で|  
|コンパイラの警告 (レベル 1) C4806|'操作': 安全でない操作: 型 'type' が 'type' を型に上位変換の値が指定した定数を等しくないです。|  
|コンパイラの警告 (レベル 1) C4807|'操作': 型 'type' および 'type' 型の符号付きビット フィールドの安全でないミックス|  
|コンパイラの警告 (レベル 1) C4808|'value' の場合はブール型のスイッチの状態の有効な値はありません。|  
...
|コンパイラの警告 (レベル 1) C4813|'function': ローカル クラスのフレンド関数必要がありますが宣言されていません|  
|コンパイラの警告 (レベル 4) C4816|'param': パラメーター (ない場合、オブジェクトが参照によって渡される) は切り捨てられますが、サイズが&0; の配列には|  
|コンパイラの警告 (レベル 1) C4817|'メンバー': 不適切に使用 '.'; このメンバーにアクセスするにはコンパイラの '->' に置き換えられます|  
...
|コンパイラの警告 (レベル 1) C4822|'メンバー関数': ローカル クラス メンバー関数には、本文はありません。|  
|[コンパイラの警告 (レベル 3) C4823](../../error-messages/compiler-warnings/compiler-warning-level-3-c4823.md)|'function': を使用して固定ポインターは、アンワインド セマンティクスが有効になっていません。 /EHa の使用を検討してください。|  
|コンパイラの警告 (レベル 4) C4825||  
|コンパイラの警告 (レベル 3) C4827|パラメーターが指定されていない 'ToString' パブリック メソッドは、virtual および override に設定する必要があります|  
|[コンパイラの警告 (レベル 1) C4829](../../error-messages/compiler-warnings/compiler-warning-level-1-c4829.md)|関数 main への正しくないパラメーターである可能性があります。 検討 ' int メイン (platform::array\<platform::string ^ > ^ argv)'|  
|[コンパイラの警告 (レベル 1) C4835](../../error-messages/compiler-warnings/compiler-warning-level-1-c4835.md)|'variable': マネージ コードが最初にホスト アセンブリで実行されるまで、エクスポートされたデータの初期化子は実行されません|  
|[コンパイラの警告 (レベル 1) C4838](../../error-messages/compiler-warnings/compiler-warning-level-1-c4838.md)|'type_1' から 'type_2' への変換が縮小変換が必要です。|  
...
|[コンパイラの警告 C4868](../../error-messages/compiler-warnings/compiler-warning-c4868.md)|'file(line_number)' コンパイラは中かっこで囲んだ初期化リスト内で左から右へ評価順序を強制しない場合があります。|  
...
|コンパイラの警告 (レベル 1) C4880|'const type_1' から 'type_2' へのキャスト: amp 制限関数で未定義の動作へのポインターまたは参照から constness のキャストがあります|  
|コンパイラの警告 (レベル 4) C4881|コンス トラクターやデストラクターは呼び出されません tile_static 変数 '変数'|  
|コンパイラの警告 (レベル 1) C4882|非 const 呼び出し演算子を使用した関数を concurrency::parallel_for_each に渡す処理は推奨されません|  
|コンパイラの警告 C4900|'Tool1' version 'version1' と 'tool2' の version 'version2' の間での Il が一致しません|  
...
|コンパイラの警告 (レベル 1) C4910|'\<識別子 >: '関数' と 'extern' が明示的にインスタンス化時に互換性がないです。|  
...
|[コンパイラの警告 (レベル 1) C4917](../../error-messages/compiler-warnings/compiler-warning-level-1-c4917.md)|'declarator': GUID だけに関連付けることにより、クラス、インターフェイス、または名前空間|  
|コンパイラの警告 (レベル 4) C4918|'character': プラグマ最適化リストに無効な文字|  
|コンパイラの警告 (レベル 1) C4920|列挙型の列挙型メンバー member_1 列挙型の列挙型では member_2 として既に認識 value_1 の = = value_2|  
|コンパイラの警告 (レベル 3) C4921|'%s': 属性の値 '%s' を複製指定しないでください。|  
|コンパイラの警告 (レベル 1) C4925|'method': dispinterface メソッドはスクリプトから呼び出すことはできません|  
|コンパイラの警告 (レベル 1) c4926 警告|'identifier': シンボルは既に定義されています。属性は無視されます。|  
...
|[コンパイラの警告 (レベル 1) C4929](../../error-messages/compiler-warnings/compiler-warning-level-1-c4929.md)|'file': タイプ ライブラリには、共用体が含まれています。'embedded_idl' 修飾子を無視します。|  
|[コンパイラの警告 (レベル 1) C4930](../../error-messages/compiler-warnings/compiler-warning-level-1-c4930.md)|'prototype': プロトタイプ宣言された関数が呼び出されません (でした変数の定義が目的としていますか?)|  
...
|コンパイラの警告 (レベル 4) C4932|__identifier(identifier) と\__identifier(identifier) は区別できません。|  
...
|コンパイラの警告 (レベル 4) c4937 警告|'text1' と 'text2' を、'directive' への引数として区別できません|  
|コンパイラの警告 (レベル 4) C4938|'var': 浮動ポイント リダクション変数/fp で一貫性のない結果が発生する可能性があります厳密なまたは #pragma fenv_access。|  
|コンパイラの警告 C4939|#プラグマ vtordisp は廃止されており、Visual C の将来のリリースで削除される予定|  
|コンパイラの警告 (レベル 1) C4944|'symbol': 'assembly1' を使用してシンボルをインポートすることはできません: 'symbol' が現在のスコープ内に既に存在します。|  
|[コンパイラの警告 (レベル 1) C4945](../../error-messages/compiler-warnings/compiler-warning-level-1-c4945.md)|'symbol': 'assembly1' を使用してシンボルをインポートすることはできません別のアセンブリ 'assembly2' から 'symbol' は既にインポートされて、。|  
...
|コンパイラの警告 (レベル 1) c4947 警告|'type_or_member': 不使用とマーク|  
|[コンパイラの警告 (レベル 2) C4948](../../error-messages/compiler-warnings/compiler-warning-level-2-c4948.md)|'アクセサー' の戻り値の型が対応する setter の最後のパラメーターの型と一致しません|  
|[コンパイラの警告 (レベル 1 およびレベル 4) C4949](../../error-messages/compiler-warnings/compiler-warning-level-1-and-level-4-c4949.md)|コンパイルした場合にのみ意味が通るプラグマ 'マネージ' と 'アンマネージ' '/clr [: オプション]'|  
|コンパイラの警告 (レベル 1) C4950|'type_or_member': 不使用とマーク|  
...
|コンパイラの警告 C4953|プロファイル データが収集されたプロファイル データを使用しませんからインライン '関数' が編集されました。|  
|コンパイラの警告 C4954|'function': プロファイルされません (_ _int64 switch 式が含まれています)|  
|コンパイラの警告 C4955|'import2': インポートが無視されます。既に 'import1' からインポート|  
|コンパイラの警告 (レベル 1) C4956|'type': この型は、検証できません|  
|コンパイラの警告 (レベル 1) C4957|'cast': 'へのキャストから' 'cast_to' からの明示的なキャストは、検証できません|  
|コンパイラの警告 (レベル 1) C4958|'操作': ポインターの算術演算は、検証できません|  
|コンパイラの警告 (レベル 1) C4959|メンバーへのアクセスで生成コードの検証できないために、/clr:safe にアンマネージ型 'type' を定義できません。|  
...
|コンパイラの警告 C4963|'%s': プロファイル データが見つかりません。別のコンパイラ オプションは、インストルメント化されたビルドで使用されました|  
...
|コンパイラの警告 (レベル 1) C4971|引数の順序:\<ターゲット オブジェクト >、\<関数をターゲット > デリゲート コンス トラクターが非推奨の使用\<関数をターゲット >、\<ターゲット オブジェクト ="">|  
...
|[コンパイラの警告 (レベル 3) C4995](../../error-messages/compiler-warnings/compiler-warning-level-3-c4995.md)|'function': 名前が非推奨 #pragma とマークされました|  

...
## <a name="remarks"></a>コメント  
 この演算子を使用すると、所有されている COM オブジェクトのメソッドを呼び出すことができます。 一時的な返します`smart_com_ptr`自動的に処理する独自`AddRef`と`Release`です。  
  
## <a name="example"></a>例  
 この例を使用して CLR クラスを実装して、`com::ptr`をそのプライベート メンバーをラップする`IXMLDOMDocument`オブジェクト。 `WriteDocument`関数を使用して`operator->`を呼び出して、`get_firstChild`ドキュメント オブジェクトのメンバーです。  

---
title: "マネージ コードからのネイティブ関数の呼び出し | Microsoft Docs"
ms.custom: ""
ms.date: "11/04/2016"
ms.reviewer: ""
ms.suite: ""
...
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "呼び出し (ネイティブ関数をマネージ コードから)"
  - "相互運用 [C++], 呼び出し (ネイティブ関数をマネージ コードから)"
  - "相互運用性 [C++], 呼び出し (ネイティブ関数をマネージ コードから)"
  - "マネージ コード [C++], 相互運用性"
  - "ネイティブ関数 (呼び出し元がマネージ コードの) [C++]"
...
> [!NOTE]
>  マーシャリング ライブラリにより、最適化された方法でネイティブ環境とマネージ環境との間でデータ変換を行うことができるようになります。  マーシャリング ライブラリの詳細については、「[C\+\+ におけるマーシャリングの概要](../dotnet/overview-of-marshaling-in-cpp.md)」を参照してください。  マーシャリング ライブラリは関数には使用できず、データにしか使用できません。  
...
```  
 ...
 このトピックで既に述べたように、マーシャリング ライブラリにより、最適化された新しい方法でネイティブ環境とマネージ環境との間でデータ変換を行うことができます。  詳細については、「[C\+\+ におけるマーシャリングの概要](../dotnet/overview-of-marshaling-in-cpp.md)」を参照してください。  

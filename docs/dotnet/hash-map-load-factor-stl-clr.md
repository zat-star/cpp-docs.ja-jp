...
## 解説  
 このメンバー関数は `(float)`[hash\_map::size](../dotnet/hash-map-size-stl-clr.md)`() /`[hash\_map::bucket\_count](../dotnet/hash-map-bucket-count-stl-clr.md)`()`を返します。  平均バケットのサイズを決定するために使用します。  

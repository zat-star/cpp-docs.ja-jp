...
## 解説  
 このメンバー関数は、反復子 `cliext::pair<iterator, iterator>(`[hash\_map::lower\_bound](../dotnet/hash-map-lower-bound-stl-clr.md)`(``key``),`[hash\_map::upper\_bound](../dotnet/hash-map-upper-bound-stl-clr.md)`(``key``))`のペアを返します。  指定したキーに一致する、被制御シーケンス内の要素の範囲を現在特定するときに使用します。  

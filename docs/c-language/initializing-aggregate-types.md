 ...
 *initializer\-list*  **,**  *initializer*  
 ...
 共用体が初期化される場合、初期化子リストは単一の定数式である必要があります。  定数式の値は共用体の最初のメンバーに割り当てられます。  
 ...
 **Microsoft 固有の仕様 →**  
 ...
 配列の最大サイズは **size\_t** によって定義されます。  STDDEF.H ヘッダー ファイルに定義されている **size\_t** は、0x00000000 ～ 0x7CFFFFFF の範囲の `unsigned int` です。  
...
```  
 ...
 次の例では、`x` の `int` の 3 つのメンバーが 1、2、および 3 にそれぞれ初期化されます。  

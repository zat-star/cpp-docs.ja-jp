...
C プログラムのステートメントは、プログラムの実行フローを制御します。  他のプログラミング言語と同様に C では、複数種類のステートメントでループを実行し、実行する他のステートメントを選択して、制御を移すことができます。  このセクションでは、簡単な「[ステートメントの構文の概要](../Topic/Overview%20of%20C%20Statements.md)」の後に、C ステートメントをアルファベット順に説明します。  

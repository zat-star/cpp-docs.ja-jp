...
|[CDacl::AddDeniedAce](#adddeniedace)|拒否されたエースからの追加、`CDacl`オブジェクト。|  
...
> [!NOTE]
>  2 番目の形式の`AddAllowedAce`は Windows 2000 で利用可能な以降のみです。  
...
> [!NOTE]
>  2 番目の形式の`AddDeniedAce`は Windows 2000 で利用可能な以降のみです。  

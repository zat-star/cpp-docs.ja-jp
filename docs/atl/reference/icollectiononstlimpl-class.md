...
---
# <a name="icollectiononstlimpl-class"></a>単独クラス
このクラスは、コレクション クラスによって使用されるメソッドを提供します。  
...
```
STDMETHOD(getcount) (時間の長い * pcount) です。
```  
...
```
STDMETHOD(get_Item) (長いインデックス、ItemType * pvar) です。
```  
...
```
STDMETHOD(get__NewEnum) (IUnknown * * ppUnk) です。
```  

...
> [!NOTE]
> `IDispEventImpl`および`IDispEventSimpleImpl`の独自の実装を提供**iunknown::queryinterface**それぞれを有効にする`IDispEventImpl`と`IDispEventSimpleImpl`基底クラスへの直接アクセスを許可する一方、個別の COM id として機能するクラスメインの COM オブジェクト内のメンバー。  

---
title: "コンストラクター (C++) | Microsoft Docs"
ms.custom: ""
ms.date: "12/03/2016"
...
-   [メンバー リスト](../cpp/constructors-cpp.md#member_lists)  
  
-   [明示的なコンストラクター](../cpp/constructors-cpp.md#explicit_constructors)  
  
-   [既定のコンストラクター](../cpp/constructors-cpp.md#default_constructors)  
  
-   [コピー コンストラクターと移動コンストラクター](../cpp/constructors-cpp.md#copy_and_move_constructors)  
  
-   [明示的に既定化および削除したコンストラクター](../cpp/constructors-cpp.md#explicitly_defaulted_and_deleted_constructors)  
  
-   [派生クラスのコンストラクター](../cpp/constructors-cpp.md#constructors_in_derived_classes)  
  
-   [多重継承を使用するクラスのコンストラクター](../cpp/constructors-cpp.md#constructors_for_classes_that_have_multiple_inheritance)  
  
-   [コンストラクターの仮想関数](../cpp/constructors-cpp.md#virtual_functions_in_constructors)  
  
-   [コンストラクターと複合クラス](../cpp/constructors-cpp.md#constructors_in_composite_classes)  
  
-   [デリゲート コンストラクター](../cpp/constructors-cpp.md#delegating_constructors)  
  
-   [コンストラクターの継承 (C++11)](../cpp/constructors-cpp.md#inheriting_constructors)  
  
-   [コンストラクターの宣言に関する規則](../cpp/constructors-cpp.md#rules_for_declaring_constructors)  
  
-   既定のコンストラクターとコピー コンストラクター  
  
-   [コンストラクターの明示的な呼び出し](../cpp/constructors-cpp.md#explicitly_invoking_constructors)  
...
##  <a name="member_lists"></a> メンバー リスト  
 コンストラクター引数のクラス メンバーを、メンバー初期化子リストを使用して初期化します。  この方法では*直接の初期化*を使用します。これは、コンストラクターの本文内で代入演算子を使用するよりも効率的です。  
...
```  
 ...
 コンストラクターが明示的な場合、次の行ではコンパイラ エラーが発生します: `ShippingOrder so(42, 10.8);`。  詳細については、「[変換](../cpp/user-defined-type-conversions-cpp.md)」を参照してください。  
  
##  <a name="default_constructors"></a> 既定のコンストラクター  
 *既定のコンストラクター*にはパラメーターがありません。これらは若干異なる規則に従っています。  
  
 既定のコンストラクターは、*特殊なメンバー関数*の一種です。クラスにコンストラクターが宣言されていない場合、コンパイラは既定のコンストラクターを指定します。  
...
```  
 ...
 これは最も厄介な解析の例です。  例の式は、関数の宣言としても、既定のコンストラクターの呼び出しとしても解釈できるため、さらに、C\+\+ パーサーが宣言を最優先に処理するため、この式は関数宣言として扱われます。  詳細については、「[Most Vexing Parse \(最も厄介な解析\)](http://en.wikipedia.org/wiki/Most_vexing_parse)」を参照してください。  
...
##  <a name="copy_and_move_constructors"></a> コピー コンストラクターと移動コンストラクター  
 *コピー コンストラクター*は、入力として同じ型のオブジェクトへの参照を受け取り、そのコピーを作成する特殊なメンバー関数です。  詳細については、「[コピー コンストラクターとコピー代入演算子 \(C\+\+\)](../cpp/copy-constructors-and-copy-assignment-operators-cpp.md)」を参照してください。  移動コンストラクターも特殊なメンバー関数であり、既存のオブジェクトの所有権を新しい変数に移動します。元のデータはコピーされません。  詳細については、「[移動コンストラクターと移動代入演算子 \(C\+\+\)](http://msdn.microsoft.com/ja-jp/1442de5f-37a5-42a1-83a6-ec9cfe0414db)」、および「[移動コンストラクターと移動代入演算子 \(C\+\+\)](../Topic/Move%20Constructors%20and%20Move%20Assignment%20Operators%20\(C++\).md)」を参照してください。  
  
##  <a name="explicitly_defaulted_and_deleted_constructors"></a> 明示的に既定化および削除したコンストラクター  
 コピー コンストラクター、既定のコンストラクター、移動コンストラクター、コピー代入演算子、移動代入演算子、およびデストラクターの明示的な既定化が可能です。  特殊なメンバー関数はすべて明示的に削除できます。  詳細については、「[明示的に既定された関数および削除された関数](../Topic/Explicitly%20Defaulted%20and%20Deleted%20Functions.md)」を参照してください。  
  
##  <a name="constructors_in_derived_classes"></a> 派生クラスのコンストラクター  
...
##  <a name="virtual_functions_in_constructors"></a> コンストラクターの仮想関数  
 ...
 *デリゲート コンストラクター*は同じクラスの別のコンストラクターを呼び出して、初期化の処理の一部を実行します。  次の例では、派生クラスに 3 つのコンストラクターがあります。2 番目のコンストラクターは最初のコンストラクターに委任し、3 番目のコンストラクターは 2 番目のコンストラクターに委任します。  
...
```  
 ...
 詳細については、「[均一な初期化とコンストラクターのデリゲート](../cpp/uniform-initialization-and-delegating-constructors.md)」を参照してください。  
...
##  <a name="rules_for_declaring_constructors"></a> コンストラクターの宣言に関する規則  
 コンストラクターはそのクラスと同じ名前になります。  コンストラクターは、オーバーロードされた関数の規則に従っていくつでも宣言できます。  詳細については、「[オーバーロード](../misc/overloading-cpp.md)」を参照してください。  
...
|コピー コンストラクター|同じクラス型への参照の単一の引数を受け取ることができます。|クラス型のオブジェクトをコピーします。|  
 ...
 stdcall 呼び出し規約は **\_\_stdcall** キーワードで宣言した静的メンバー関数とグローバル関数で使用され、変数引数リストを使用しません。  非静的メンバー関数 \(コンストラクターなど\) で **\_\_stdcall** キーワードを使用すると、コンパイラで thiscall の呼び出し規約が使用されます。  
  
 基底クラスのコンストラクターは、派生クラスによって継承されません。  派生クラス型のオブジェクトが作成されるときは、最初に基底クラスのコンポーネントの構築が行われ、その後、派生クラスのコンポーネントに移ります。  コンパイラは、完全なオブジェクトのその部分が初期化されているものとして、各基底クラスのコンストラクターを使用します \(仮想派生の場合は除きます。これについては、「[Initializing Base Classes \(基底クラスの初期化\)](../misc/initializing-base-classes.md)」を参照\)。  
  
##  <a name="explicitly_invoking_constructors"></a> コンストラクターの明示的な呼び出し  
...
```  
 ...
 前の 2 つの例のように、コンストラクターを明示的に呼び出すことによって作成されたオブジェクトには名前がなく、オブジェクトを作成する式には有効期間があります。  これについては、「[一時オブジェクト](../cpp/temporary-objects.md)」で詳しく説明します。  

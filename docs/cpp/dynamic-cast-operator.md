...
## 解説  
 ...
 静的キャスト変換と動的キャスト変換の違い、およびそれぞれの適切な用途については、「[static\_cast](../cpp/static-cast-operator.md)」を参照してください。  
...
```  
 ...
 CLR 型の場合、`dynamic_cast` を実行すると、暗黙的な変換が可能な場合、何も行われません。変換が失敗した場合は、動的チェックを実行し、`nullptr` を返す MSIL `isinst` 命令が実行されます。  
...
```  
 ...
 ![複数の継承を示すクラスの階層構造](../cpp/media/vc39011.png "vc39011")  
複数の継承を示すクラスの階層構造  
...
```  
 ...
 ![仮想基底クラスを示すクラスの階層構造](../cpp/media/vc39012.png "vc39012")  
仮想基底クラスを示すクラスの階層構造  
  
 この階層構造では、`A` は仮想基底クラスです。  仮想基底クラスの定義については、「[仮想基底クラス](../Topic/Virtual%20Base%20Classes.md)」を参照してください。  `E` クラスのインスタンスと `A` サブオブジェクトへのポインターがある場合、`B` へのポインターへの `dynamic_cast` はあいまいさが原因で失敗します。  最初に、完全な `E` オブジェクトにキャストし、その後、明確な方法で階層を上へとたどり、正しい `B` オブジェクトに到達する必要があります。  
  
 次の図に示すクラスの階層構造の場合を考えてみましょう。  
  
 ![重複基底クラスを示すクラスの階層構造](../Image/vc39013.gif "vc39013")  
基底クラスの複製を示すクラスの階層構造  
...
```  
 ...
 `dynamic_cast` 演算子を使用して、"クロス キャスト" を実行することもできます。 同じクラス階層を使用すると、完全なオブジェクトが `E` 型であれば、たとえば `B` サブオブジェクトから `D` サブオブジェクトへのように、ポインターをキャストできます。  
...
```  
 ...
 ポインター型への失敗したキャストの値は、null ポインターです。  参照型へのキャストが失敗すると、[bad\_cast 例外](../cpp/bad-cast-exception.md)がスローされます。`expression` が有効なオブジェクトを指していない、または参照していない場合は、`__non_rtti_object` 例外がスローされます。  
  
 `__non_rtti_object` 例外の詳細については、「[typeid](../cpp/typeid-operator.md)」を参照してください。  

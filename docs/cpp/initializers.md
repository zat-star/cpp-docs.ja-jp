...
-   配列 \([POD](../standard-library/is-pod-class.md) クラス、構造体、共用体\) は、そのメンバーがゼロ値に初期化されます。  
...
```  
 ...
 グローバルな静的オブジェクトの初期化の詳細については、「[起動に関するその他の考慮事項](../cpp/additional-startup-considerations.md)」を参照してください。  
...
-   クラス、構造体、共用体のメンバーが集約の初期化時にコピー初期化によって初期化される。  例については、「[集約の初期化](../cpp/initializers.md#INIT_AggInit)」を参照してください。  
...
```  
 ...
 場合によっては、クラスのコピー コンストラクターが削除されるかアクセスできないと、コピー初期化によりコンパイラ エラーが発生します。  詳細については、「[\(NOTINBUILD\) Explicit Initialization](http://msdn.microsoft.com/ja-jp/c89724f8-ddd3-4d77-b86d-77fcd8bd8595)」を参照してください。  
...
-   `extern` として明示的に指定される変数の宣言。  例:  
 ...
 ![参照型の初期化の判定グラフ](../cpp/media/vc38s71.png "vc38S71")  
参照型の初期化の判定グラフ  
  
 \(`volatile` *typename***&** *identifier* として宣言された\) `volatile` 型への参照は、同じ型の `volatile` オブジェクト、または `volatile` として宣言されていないオブジェクトで初期化できます。  ただし、同じ型の **const** オブジェクトにより初期化することはできません。  同様に、\(**const** *typename***&***identifier*  として宣言された\) **const** 型に対する参照は、同じ型の **const** オブジェクト \(またはその型への変換を含むすべての要素、あるいは **const** として宣言されていないオブジェクトによる変換を含むすべての要素\) により初期化できます。  ただし、同じ型の `volatile` オブジェクトにより初期化することはできません。  
  
 **const** キーワードまたは `volatile` キーワードのいずれによっても修飾されていない参照は、**const** または `volatile` のいずれとしても宣言されていないオブジェクトによってのみ初期化できます。  
  
### 外部変数の初期化  
 自動変数、レジスタ変数、静的変数、外部変数の宣言には、初期化子を含めることができます。  ただし、外部変数の宣言では、変数が `extern` として宣言されていない場合にのみ初期化子を含めることができます。  詳細については、「[\[外部\]](../misc/external.md)」を参照してください。  
  
## 参照  
 [\(NOTINBUILD\)Declarators](http://msdn.microsoft.com/ja-jp/8a7b9b51-92bd-4ac0-b3fe-0c4abe771838)
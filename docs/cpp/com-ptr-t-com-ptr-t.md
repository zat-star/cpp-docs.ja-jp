...
#### パラメーター  
 ...
 `fAddRef`  
 **true** の場合、カプセル化されたインターフェイス ポインターの参照カウントをインクリメントするために `AddRef` が呼び出されます。  
 ...
 *lpcStr*  
 **CLSID** \(先頭は "**{**"\) または **ProgID** を保持するマルチバイト文字列。  
  
 `pOuter`  
 [集計](http://msdn.microsoft.com/library/windows/desktop/ms686558)で不明な外部の値。  

...
-   周辺デバイスとの間で送受信されるストリーム バイト。  
 ...
 対象の環境では、プログラムの起動前に 3 つのファイルを開きます。 2 つの引数を使用してライブラリ関数 [fopen、_wfopen](../c-runtime-library/reference/fopen-wfopen.md) を呼び出すことで、ファイルを開くことができます (`fopen` 関数は使用されていません。代わりに [fopen_s、_wfopen_s](../c-runtime-library/reference/fopen-s-wfopen-s.md) を使用します)。1 つ目の引数は、ファイル名です。 2 つ目の引数は、次を指定する C 文字列です。  

...
-   **new と delete** In previous versions of the library, the implementation-defined operator new と delete functions were exported from the runtime library DLL (for example, msvcr120.dll). 現在、これらの演算子関数は常に (ランタイム ライブラリ DLL を使用する場合でも) 静的にバイナリにリンクされています。  

...
イベント '\<`eventname`\>' の定義を含むアセンブリ '\<`assemblyname`\>' への参照が必要です。 プロジェクトへの参照を追加します。  
 ...
 プロジェクト内で直接参照されないダイナミック リンク ライブラリ \(DLL\) またはアセンブリでイベントが定義されています。[!INCLUDE[vbprvb](../Token/vbprvb_md.md)] コンパイラでは、イベントが複数の DLL またはアセンブリで定義されている場合に備えて、あいまいさを避けるための参照が必要になります。  

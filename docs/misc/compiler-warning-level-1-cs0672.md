...
メンバー 'member1' が古い形式のメンバー 'member2' をオーバーライドします。 Obsolete 属性を 'member1' に追加してください  
 ...
 コンパイラが `obsolete` としてマークされたメソッドへの `override` を検出しました。 ただし、オーバーライド元のメソッド自体は obsolete とマークされていませんでした。 オーバーライド元のメソッドを呼び出すと、引き続き [CS0612](../Topic/Compiler%20Warning%20\(level%201\)%20CS0612.md) が生成されます。  

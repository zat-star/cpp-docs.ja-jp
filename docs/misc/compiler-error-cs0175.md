...
キーワード 'base' の使用はこのコンテキストでは有効ではありません。  
 ...
 [base](../Topic/base%20\(C%23%20Reference\).md) キーワードを使用して、基底クラスの特定のメンバーを指定する必要があります。 詳細については、「[コンストラクター](../Topic/Constructors%20\(C%23%20Programming%20Guide\).md)」を参照してください。  

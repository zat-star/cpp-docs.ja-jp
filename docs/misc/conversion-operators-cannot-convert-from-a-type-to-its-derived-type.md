...
変換演算子にパラメーターから派生した戻り値の型が宣言されています。  
 ...
 コンパイル時に、[!INCLUDE[vbprvb](../Token/vbprvb_md.md)] では任意の参照型から継承階層内の任意の型、つまり派生元または派生先の型に存在する定義済みの変換が考慮されます。 このような変換は実行時に失敗する可能性がありますが、コンパイラは実行結果を予測できないので、これらの変換のコンパイルはすべて許可されます。  

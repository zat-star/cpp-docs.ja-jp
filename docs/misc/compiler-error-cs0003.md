...
メモリが不足しています  
 ...
 このエラーは、[!INCLUDE[winsdklong](../dotnet/includes/winsdklong_md.md)] と C\# コンパイラのバージョンが一致しなかったり、C\# コンパイラをサポートする 1 つまたは複数のファイルが破損しているために発生する場合もあります。この場合には、Visual Studio を再インストールしてください。
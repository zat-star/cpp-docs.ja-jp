...
派生クラス内のプロシージャは、基底クラスで同じ名前を持つプロシージャをオーバーライドしますが、パラメーター リストが異なります。  
 ...
 継承クラスでプロシージャをオーバーライドするには、オーバーライド元のプロシージャがパラメーター リスト、アクセス レベル、および戻り値の型 \(存在する場合\) と一致する必要があります。 特に、[Optional](../Topic/Optional%20\(Visual%20Basic\).md) または [ParamArray](../Topic/ParamArray%20\(Visual%20Basic\).md) 宣言と一致する必要があります。  
...
-   基底クラスのバージョンとは異なるパラメーター リストが必要である場合は、オーバーライドできません。 代わりにオーバーロードすることを検討してください。 詳細については、「[Procedure Overloading](../Topic/Procedure%20Overloading%20\(Visual%20Basic\).md)」を参照してください。  

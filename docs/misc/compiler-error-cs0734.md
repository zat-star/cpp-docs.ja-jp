...
\/moduleassemblyname オプションは 'module' のターゲット型をビルドするときのみ指定できます。  
 ...
 コンパイラ オプション **\/moduleassemblyname** は、.netmodule をビルドするときのみ使用する必要があります。 詳細については、「[\/moduleassemblyname \(Specify Friend Assembly for Module\)](../Topic/-moduleassemblyname%20\(C%23%20Compiler%20Option\).md)」を参照してください。  

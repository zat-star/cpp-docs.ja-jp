...
'method' は ref パラメーターの Out 属性だけを指定することはできません。 In 属性と Out 属性の両方を使用するか、どちらも使用しないでください。  
 ...
 インターフェイス メソッドには、[Out](frlrfSystemRuntimeInteropServicesOutAttributeClassTopic) 属性だけで [ref](../Topic/ref%20\(C%23%20Reference\).md) を使用するパラメーターがあります。**Out** 属性を使用する `ref` パラメーターでは、[In](frlrfSystemRuntimeInteropServicesInAttributeClassTopic) 属性も使用する必要があります。  

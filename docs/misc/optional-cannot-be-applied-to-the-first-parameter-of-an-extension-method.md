...
'Optional' は、拡張メソッドの最初のパラメーターには適用できません。 最初のパラメーターは、拡張する型を指定します。  
 ...
 この制限が適用されるのは、最初のパラメーターに対してのみです。 その他のパラメーターは省略可能な場合もそうでない場合もあり、他のメソッドと同じ規則に従います。 詳細については、「[Parameter List](../Topic/Parameter%20List%20\(Visual%20Basic\).md)」を参照してください。  

...
旧形式のメンバー 'memberA' が、旧形式でないメンバー 'memberB' をオーバーライドします。  
 ...
 通常、不使用とマークされているメンバーで不使用とマークされていないメンバーをオーバーライドしないでください。 この警告は [!INCLUDE[vs_orcas_long](../atl/reference/includes/vs_orcas_long_md.md)] で生成されますが、[!INCLUDE[vsprvslong](../error-messages/compiler-errors-1/includes/vsprvslong_md.md)] では生成されません。  

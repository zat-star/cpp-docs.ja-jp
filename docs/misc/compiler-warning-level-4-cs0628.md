...
'member': 新規のプロテクト メンバーがシール クラスで宣言されました  
 ...
 他のクラスは `sealed` クラスから継承できず、`protected` メンバーを使用することができないため、[シール](../Topic/sealed%20\(C%23%20Reference\).md) クラスは[プロテクト](../Topic/protected%20\(C%23%20Reference\).md) メンバーを使用できません。  

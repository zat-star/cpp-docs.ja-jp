...
'interface member' : 含む型は、インターフェイス 'interface' を実装しません  
 ...
 インターフェイス メンバーを、[インターフェイス](../Topic/interface%20\(C%23%20Reference\).md)から派生していない[クラス](../Topic/class%20\(C%23%20Reference\).md)に実装しようとしました。 インターフェイス メンバーの実装を削除するか、インターフェイスをクラスの基底クラス リストに追加する必要があります。  

...
既に fixed が使用されている式のアドレスを取得するために、fixed ステートメントを使用することはできません。  
 ...
 [unsafe](../Topic/unsafe%20\(C%23%20Reference\).md) メソッドまたはパラメーター内のローカル変数は既に \(スタック上に\) 固定されているため、[fixed](../Topic/fixed%20Statement%20\(C%23%20Reference\).md) 式ではこれらの 2 つの変数のアドレスを使用できません。 詳細については、「[unsafe コードとポインター](../Topic/Unsafe%20Code%20and%20Pointers%20\(C%23%20Programming%20Guide\).md)」を参照してください。  

...
stackalloc は catch または finally ブロックで使用されない可能性があります。  
 ...
 [stackalloc](../Topic/stackalloc%20\(C%23%20Reference\).md) キーワードは [catch](../Topic/try-catch%20\(C%23%20Reference\).md) または [finally](../Topic/try-catch-finally%20\(C%23%20Reference\).md) ブロックで使用できません。 詳細については、「[例外と例外処理](../Topic/Exceptions%20and%20Exception%20Handling%20\(C%23%20Programming%20Guide\).md)」を参照してください。  

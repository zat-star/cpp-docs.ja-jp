...
パラメーターのインターフェイス型を使用して変換演算子が宣言されています。  
 ...
 コンパイル時に、[!INCLUDE[vbprvb](../Token/vbprvb_md.md)] では任意のインターフェイスから参照型への変換が事前に定義されているものと見なします。 このような変換は実行時に失敗する可能性がありますが、コンパイラは実行結果を予測できないため、これらの変換のコンパイルはすべて許可されます。  

...
型引数リストの指定なくジェネリック プロシージャが呼び出され、型引数のいずれかについて型の推定が失敗します。  
 ...
 ジェネリック プロシージャを呼び出す場合、通常、型パラメーターごとにプロシージャで定義されている型引数を指定します。 ただし、型引数リストをすべて省略することもできます。 これを行う場合、コンパイラでは、呼び出しのコンテキストから、型引数ごとに型の推論が試行されます。 詳細については、「[Generic Procedures in Visual Basic](../Topic/Generic%20Procedures%20in%20Visual%20Basic.md)」の「型の推定」を参照してください。  

...
プログラム 'output file name' に、複数のエントリ ポイントが定義されています。 エントリ ポイントを含む型を指定するために、\/main を使用してコンパイルします。  
 ...
 プログラムには、[Main](../Topic/Main\(\)%20and%20Command-Line%20Arguments%20\(C%23%20Programming%20Guide\).md) メソッドを 1 つのみ指定できます。  
  
 このエラーを解決するには、Main メソッドを 1 つのみコードに残して残りをすべてコードから削除するか、[\/main](../Topic/-main%20\(C%23%20Compiler%20Options\).md) コンパイラ オプションを使用してどの Main メソッドを使用するか指定できます。  

...
変換演算子が、戻り値の型 [Object Data Type](../Topic/Object%20Data%20Type.md) を使用して宣言されます。  
  
 コンパイル時に [!INCLUDE[vbprvb](../Token/vbprvb_md.md)] では、任意の参照型から継承階層内の任意の型、つまり派生元または派生先の型に存在する定義済みの変換が考慮されます。`Object` は [!INCLUDE[dnprdnshort](../Token/dnprdnshort_md.md)] の汎用データ型であるため、すべての型が `Object` から派生します。  

...
out パラメーター 'parameter' はコントロールが現在のメソッドを抜ける前に割り当てられる必要があります  
 ...
 [out](../Topic/out%20\(C%23%20Reference\).md) キーワードでマークされたパラメーターに、メソッド本体で値が代入されませんでした。 詳細については、「[パラメーターの引き渡し](../Topic/Passing%20Parameters%20\(C%23%20Programming%20Guide\).md)」を参照してください。  

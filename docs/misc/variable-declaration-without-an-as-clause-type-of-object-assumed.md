...
変数宣言は `As` 句を指定していません。  
 ...
 `As` 句は、プログラミング要素に関連付けられるデータ型を指定します。[Dim Statement](../Topic/Dim%20Statement%20\(Visual%20Basic\).md) で、変数 \(複数可\) のデータ型が指定されます。`Dim` ステートメントに `As` 句を含めない場合、変数のデータ型は既定で `Object` になります。  

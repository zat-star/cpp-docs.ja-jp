...
`Private` で宣言されているために、このコンテキストでアクセスできないメソッドにアクセスしようとしました。 このエラーの考えられる原因は、[!INCLUDE[vbprvb](../Token/vbprvb_md.md)] コンパイラがクラスのすべてのメンバーをインポートし、大文字と小文字を区別しないため、小文字だけが異なる名前が競合する可能性があることです。  

...
set アクセサーにアクセスできないため、プロパティまたはインデクサー 'property\/indexer' はこのコンテキストでは使用できません  
 ...
 このエラーは、`set` アクセサーがプログラム コードにアクセスできないと発生します。 このエラーを解決するには、アクセサーのアクセシビリティを増やすか、呼び出し元の場所を変更します。 詳細については、「[アクセサーのアクセシビリティの制限](../Topic/Restricting%20Accessor%20Accessibility%20\(C%23%20Programming%20Guide\).md)」を参照してください。  

...
キーワード 'base' は静的メソッドでは使用できません。  
 ...
 [base](../Topic/base%20\(C%23%20Reference\).md) キーワードが [static](../Topic/static%20\(C%23%20Reference\).md) メソッドで使用されました。`base` は、インスタンス コンストラクター、インスタンス メソッド、またはインスタンス アクセサーでのみ呼び出すことができます。  

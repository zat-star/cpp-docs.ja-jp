...
C++ では、Web とクラウドに接続するためのいくつかの方法があります。  
 ...
 Windows ストア アプリまたは Windows デスクトップ アプリで、Microsoft Azure Mobile Services に接続するために使用できる、ネイティブな API を提供しています。 Web サイトの例のほとんどは C# を使用していますが、C++ も使用できます。 詳しくは、「 [クイック スタート: C++ を使ったモバイル サービスの追加](http://msdn.microsoft.com/library/windows/apps/dn263181.aspx)」をご覧ください。  
...
[IXMLHTTPRequest2 インターフェイス](http://msdn.microsoft.com/library/windows/apps/hh831151.aspx)  
 Windows ストア アプリまたは Windows デスクトップ アプリで、HTTP 上でインターネットに接続し、GET、PUT などの HTTP コマンドを発行するために使用できる、ネイティブな COM インターフェイスを提供します。 詳細については、次を参照してください。[チュートリアル: 接続を使用してタスクおよび XML HTTP 要求](../parallel/concrt/walkthrough-connecting-using-tasks-and-xml-http-requests.md)です。  
...
## <a name="see-also"></a>参照  
 ...
 [ネットワークと web サービス (C #/VB/C + + と XAML を使った Windows ストア アプリ) に接続します。](http://msdn.microsoft.com/library/windows/apps/br229573.aspx)
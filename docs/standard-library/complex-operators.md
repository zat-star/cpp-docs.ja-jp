...
`left` 複素数または非等値をテストするには、そのパラメーター型のオブジェクト。

`right` 複素数または非等値をテストするには、そのパラメーター型のオブジェクト。
...
`left` 最初の 2 つの複素数またはを使用して乗算するためには、複素数のパラメーターの型の数、* 操作します。

`right` 2 つの複素数を乗算するのには、複素数のパラメーターの型の数の 2 つ目、* 操作します。
...
`left` 最初の 2 つの複素数またはを使用して追加するのには、複素数のパラメーターの型の数、+ 操作します。

`right` 2 番目の 2 つの複素数またはを使用して追加するのには、複素数のパラメーターの型の数、+ 操作します。
...
`left` 2 つの複素数または -操作によって減算される複素数のパラメーターの型である番号の最初の数値。

`right` 2 番目の 2 つの複素数または -操作によって減算される複素数のパラメーターの型の数。
...
`left` 複素数または番号の分子と分母で割ることである複素数のパラメーターの型を持つ、/操作します。

`right` 複素数または数値を分母で分子を分割するために使用するのには、複素数のパラメーターの型の/操作します。
...
`Ostr` 複素数の値が入力されている先の出力ストリーム。

`right` 出力ストリームに入力する複素数

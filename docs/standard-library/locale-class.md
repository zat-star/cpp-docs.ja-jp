...
`Loc` ターゲット ロケールに挿入するファセットを含むロケールです。
...
`Loc` 新しいロケールを構築する際にコピーするロケールです。

`Other` カテゴリを選択するロケール。

`Cat` 構築されたロケールに置き換えられるカテゴリです。

`Fac` 構築されたロケールに置き換えられるファセット。

...
### <a name="remarks"></a>コメント  
 `difference_type` は、コンテナーの反復子を減算またはインクリメントするときに返される型です。 `difference_type`範囲内の要素の数を表すために使用が通常 [*最初*、*最後*)、反復子間`first`と`last`、によって指し示される要素が含まれています`first`要素までの範囲、要素を指すおよび`last`です。  

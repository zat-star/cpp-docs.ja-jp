...
`first` 前方反復子の範囲の最初の要素の位置を示す検索します。

`last` 前方反復、いずれかの過去の最後の要素範囲内の位置を検索します。

`comp` 検索対象の範囲内で隣接する要素の値によって満たされる条件を提供する二項述語。
...
`first` 条件のチェックを開始する場所を示す入力反復子。 反復子は、要素の範囲の開始位置をマークします。

`last` 条件をチェックする要素の範囲の末尾を示す入力反復子。

`comp` テストするための条件です。 これは、チェックされる要素によって満たされる条件を定義するユーザー定義の述語関数オブジェクトです。 述語は 1 つの引数を取り、`true` または `false` を返します。
...
`first` 範囲の要素の条件の確認を開始する場所を示す入力反復子。

`last` 条件をチェックする要素の範囲の末尾を示す入力反復子。

`comp` テストするための条件です。 これは、ユーザー定義の述語関数オブジェクトによって提供されます。 述語は、テスト対象の要素によって満たされる条件を定義します。 述語は 1 つの引数を取り、`true` または `false` を返します。
...
`value` 要素の値が一致する値が必要なまたは二項述語によって指定された要素の値の条件を満たす必要があります。

`comp` 1 つの要素が他よりも小さいが意味を定義するユーザー定義の述語関数オブジェクト。 二項述語は 2 つの引数を受け取り、条件が満たされている場合は `true`、満たされていない場合は `false` を返します。
...
`first` ソース範囲内の最初の要素の位置を示す入力反復子。

`last` 1 つ、ソース範囲の最後の要素の後ろの位置を示す入力反復子。

*destBeg*ターゲット範囲の最初の要素の位置を示す出力反復子。
...
`first` ソース範囲の最初の要素の位置を示す双方向反復子。

`last` 双方向反復子の 1 つ、ソース範囲の最後の要素の後ろの位置を示すです。

`destEnd` ターゲット範囲内の 1 つ後ろの末尾の要素の位置を示す双方向反復子。
...
`first` 条件を確認する範囲の始まりを示す入力反復子。

`last` 範囲の終了を示す入力反復子。

`dest` コピーした要素のシリアル化先を示す出力反復子。

`_Pred` 範囲内のすべての要素を評価する条件です。 この条件は、ユーザー定義の述語関数オブジェクトによって提供されます。 述語は 1 つの引数を取り、`true` または `false` を返します。
...
`first` 要素をコピーする場所を示す入力反復子。

`count` コピーする要素の数を指定する符号付きまたは符号なし整数型。

`dest` 要素をコピーする場所を示す出力反復子。
...
`first` 入力反復子の範囲内の最初の要素の位置が走査されます。

`last` 入力反復子のいずれかの後ろの位置、最後の要素範囲が走査されます。
...
`first` 入力反復子の範囲内の最初の要素の位置を検索します。

`last` 入力反復子、いずれかの過去の最後の要素範囲内の位置を検索します。

`_Pred` 要素がカウントする場合に満たされる条件を定義するユーザー定義の述語関数オブジェクトです。 述語は 1 つの引数を取り、**true** または **false** を返します。
...
`First1` 入力反復子の最初の範囲の最初の要素の位置を示すテストします。

`Last1` 入力反復子、最初の範囲の最後の要素の後ろの 1 つの位置を示すテストします。

`First2` 入力反復子の 2 つ目の範囲内で最初の要素の位置を示すテストします。

`First2` 入力反復子の 2 つ目の範囲内で 1 つ後ろの最後の要素の位置を示すテストします。

`Comp` 2 つの要素が実行される場合に満たされる条件を定義するユーザー定義の述語関数オブジェクトと等価です。 二項述語は 2 つの引数を受け取り、条件が満たされている場合は **true** 、満たされていない場合は **false** を返します。
...
`val` 順序付けられた範囲内で検索される値です。

`comp` 1 つの要素が他よりも小さいが意味を定義するユーザー定義の述語関数オブジェクト。
...
`first` 前方反復子の範囲の最初の要素の位置を示すために走査しなければです。

`last` 前方反復、いずれかの過去の最後の要素範囲内の位置が走査されます。

`val` 範囲内の要素に割り当てられる値 [ `first`、 `last`)。
...
`First` 出力反復子の範囲の最初の要素の位置を示す値を割り当てる`Val`です。

`Count` 値が割り当てられる要素の数を指定する符号付きまたは符号なし整数型。

`Val` 範囲内の要素に割り当てられる値 [ `First`、 *first+count*)。
...
`first` 入力反復子の範囲の最初の要素の位置を示す、指定された値を検索します。

`last` 入力反復子範囲の最後の要素の後ろの 1 つの位置を示す、指定された値を検索します。
...
`First1` 前方反復子の範囲の最初の要素の位置を示す検索します。

`Last1` 前方反復子のいずれかの後ろの位置の最後の要素範囲のアドレス指定も検索します。

`First2` 前方反復子の範囲の最初の要素の位置を示すを検索します。

`Last2` 前方反復いずれかの後ろの位置の最後の要素範囲を検索します。
...
`last1` 前方反復、いずれかの過去の最後の要素範囲内の位置を検索します。

`first2` 前方反復子の範囲の最初の要素の位置を示すと照合されます。

`last2` 前方反復、いずれかの過去の最後の要素範囲内の位置と照合されます。
...
`pred` ユーザー定義の述語関数オブジェクトまたは[ラムダ式](../cpp/lambda-expressions-in-cpp.md)検索対象の要素によって満たされる条件を定義します。 述語は 1 つの引数を受け取り、`true` (条件が満たされた場合) または `false` (条件が満たされなかった場合) を返します。 `pred` の署名は、実質的には `bool pred(const T& arg);` にする必要があります。`T` は、`InputIterator` が逆参照されたときの暗黙的な変換先となる型です。 `const` キーワードは、関数オブジェクトまたはラムダを変更しないようにする必要があることを示すためにのみ指定します。
...
`pred` ユーザー定義の述語関数オブジェクトまたは[ラムダ式](../cpp/lambda-expressions-in-cpp.md)検索対象の要素が満たさない条件を定義します。 述語は 1 つの引数を受け取り、`true` (条件が満たされた場合) または `false` (条件が満たされなかった場合) を返します。 `pred` の署名は、実質的には `bool pred(const T& arg);` にする必要があります。`T` は、`InputIterator` が逆参照されたときの暗黙的な変換先となる型です。 `const` キーワードは、関数オブジェクトまたはラムダを変更しないようにする必要があることを示すためにのみ指定します。
...
`first` 処理される範囲内の最初の要素の位置を示す入力反復子。

`last` 1 つの後ろの位置、最後の要素範囲を示す入力反復処理します。

`_Func` 範囲内の各要素に適用されるユーザー定義の関数オブジェクトです。
...
`first` 値が割り当てられる範囲内の最初の要素の位置を示す前方反復子。

`last` 過去の値が割り当てられる範囲の最後の要素のいずれかの位置を示す前方反復子。

`_Gen` 関数オブジェクト引数なしで呼び出される各範囲内の要素に割り当てられる値を生成するために使用します。
...
`First` 値が割り当てられる範囲内の最初の要素の位置を示す出力反復子。

`Count` ジェネレーター関数から値を代入する要素の数を指定する符号付きまたは符号なし整数型。

`Gen` 関数オブジェクト引数なしで呼び出される各範囲内の要素に割り当てられる値を生成するために使用します。
...
`first1` 最初の 2 つ目のすべての要素が含まれるかどうかをテストする 2 つの並べ替えられたソース範囲の最初の最初の要素の位置を示す入力反復子。

`last1` 示す入力反復子いずれかの後ろの位置の最後の要素の最初の 2 つの並べ替えられたソース範囲の最初の 2 つ目のすべての要素が含まれるかどうかをテストします。

`first2` 2 つの 1 秒間の最初の要素の位置を示す入力反復子連続する並べ替え済みソース範囲の最初の 2 つ目のすべての要素が含まれるかどうかをテストします。

`last2` 最初の 2 つ目のすべての要素が含まれるかどうかをテストする 2 つの連続する並べ替えられたソース範囲の 1 秒間後ろの最後の要素のいずれかの位置を示す入力反復子。
...
`first` 最初の 2 つの連続する内の最初の要素の位置を示す双方向反復子には、結合して 1 つの範囲に分類する範囲が並べ替えられます。

`middle` 2 つの連続する 2 番目の最初の要素の位置を示す双方向反復子には、結合して 1 つの範囲に分類する範囲が並べ替えられます。

`last` 1 つの位置を過去の最後の要素の 2 つの連続する 2 番目のアドレス指定する双方向反復子には、結合して 1 つの範囲に分類する範囲が並べ替えられます。

`comp` 1 つの要素が他よりも大きいという意味を定義するユーザー定義の述語関数オブジェクト。 2 項述語は 2 つの引数を受け取り、最初の要素が 2 番目の要素より小さい場合は **true** 、そうでない場合は **false** を返す必要があります。
...
`first` ヒープを確認する範囲の始まりを示すランダム アクセス反復子。

`last` 範囲の終了を示すランダム アクセス反復子。

`comp` 要素の順序をテストする条件です。 二項述語は 1 つの引数を返します`true`または`false`です。
...
`begin` ヒープを確認する範囲の最初の要素を指定するランダム アクセス反復子。

`end` ヒープを確認する範囲の終わりを指定するランダム アクセス反復子。

`compare` 厳密弱順序の条件を指定する二項述語では、ヒープを定義します。 `compare` を指定しない場合の既定の述語は `std::less<>` です。
...
`first` 条件を確認する範囲の開始位置を示す入力反復子。

`last` 範囲の終了を示す入力反復子。

`comp` テストする条件です。 これは、検索対象の要素によって満たされる条件を定義するユーザー定義の述語関数オブジェクトによって提供されます。 述語は 1 つの引数を取り、`true` または `false` を返します。
...
`First1` 範囲の最初の要素を参照する前方反復子。

`Last1` 1 つ後ろの範囲の最後の要素を参照する前方反復子。

`First2` 比較に使用される 2 番目の範囲の最初の要素を参照する前方反復子。

`Last2` 1 つ後ろの比較に使用される、2 つ目の範囲の最後の要素を参照する前方反復子。

`Pred` 等価性をテストし、返します述語、`bool`です。
...
`first` 確認する範囲の開始位置を示す前方反復子。

`last` 範囲の終了を示す前方反復子。

`comp` 2 つの要素間の順序を決定するためのテスト条件です。 述語は 1 つの引数を取り、`true` または `false` を返します。 これは `operator<` と同じタスクを実行します。
...
`comp` 2 つの要素間の順序を決定するためのテスト条件です。 述語は 1 つの引数を取り、`true` または `false` を返します。
...
`left` 値がある、交換する前方反復子の 1 つ。

`right` 前方反復子の 2 番目の値を持つは、交換するのにです。
...
`first1` 入力反復子、最初の範囲内の最初の要素の位置と比較します。

`last1` 入力反復子の 1 つ位置過去の最初の範囲の最後の要素と比較します。

`first2` 入力反復子の最初の要素の 2 つ目の範囲内の位置と比較します。

`last2` 入力反復子の 1 つ位置過去の 2 番目の範囲の最後の要素と比較します。
...
`value` 最初の位置や可能な最初の位置は検索対象の順序付けられた範囲の値です。
...
`first` ヒープに変換する対象の範囲内の最初の要素の位置を示すランダム アクセス反復子。

`last` 過去のあるヒープに変換する範囲の最後の要素のいずれかの位置を示すランダム アクセス反復子。
...
`left` 比較対象となる 2 つのオブジェクトの最初の数値。

`right` 比較対象となる 2 つのオブジェクトの 2 番目のです。

`comp` 2 つのオブジェクトを比較するために使用する二項述語。

`_IList` 比較するオブジェクトを含む初期化子リスト。
...
`first` 前方反復子の範囲の最初の要素の位置を示す最大の要素を検索します。

`last` 前方反復子範囲の最後の要素の後ろの 1 つの位置を示す最大の要素を検索します。
...
`first1` 結合して 1 つの範囲に分類する 2 つの並べ替えられたソース範囲の最初の最初の要素の位置を示す入力反復子。

`last1` 1 つの後ろの位置の最後の要素、最初の結合して 1 つの範囲に分類する 2 つの並べ替えられたソース範囲を示す入力反復子。

`first2` 1 秒間の結合して 1 つの範囲に分類する 2 つの連続する並べ替えられたソース範囲の最初の要素の位置を示す入力反復子。

`last2` 過去の最後の要素の 1 秒間の結合して 1 つの範囲に分類する 2 つの連続する並べ替えられたソース範囲の 1 つの位置を示す入力反復子。

`result` 2 つのソース範囲が単一の並べ替えられた範囲に結合されるターゲット範囲内の最初の要素の位置を示す出力反復子。
...
`_IList` 比較されるメンバーを含む initializer_list。
...
`first` 前方反復子の範囲の最初の要素の位置を示す最小の要素を検索します。

`last` 前方反復子範囲の最後の要素の後ろの 1 つの位置を示す最小の要素を検索します。
...
`Last2` 入力反復子の 2 つ目の範囲内で 1 つ後ろの最後の要素の位置を示すテストします。

`Comp` 各範囲内の現在の要素を比較し、それらが等しいかどうかを判断するユーザー定義の述語関数オブジェクトです。 条件が満たされている場合は **true**、満たされていない場合は **false** を返します。
...
`first` 場所に移動する要素の範囲の開始を示す入力反復子。

`last` 移動する要素の範囲の末尾を示す入力反復子。

`dest` 移動された要素を格納する出力反復子。
...
`first` 要素を移動する範囲の始まりを示す反復子。

`last` 要素を移動する範囲の終了を示す反復子。 この要素は移動されません。
...
`first` 双方向反復子の範囲の最初の要素の位置を指定する変えるします。

`last` 双方向反復子、いずれかの過去の最後の要素範囲内の位置を指す変えるされます。

`comp` 順序の一連の要素によって満たされると、比較条件を定義するユーザー定義の述語関数オブジェクト。 二項述語は 2 つの引数を受け取り、条件が満たされている場合は **true** 、満たされていない場合は **false** を返します。
...
`first` ランダム アクセス反復子の範囲内の最初の要素の位置をパーティション分割します。

*_Nth*ランダム アクセス反復子の要素の位置をアドレス指定をパーティションの境界に正しく順序付けます。

`last` ランダム アクセス反復子の 1 つ位置過去の範囲の最後の要素を分割します。
...
`first` 条件の要素の範囲のチェックを開始する場所を示す入力反復子。

`last` 要素の範囲の終了を示す入力反復子。

`comp` テストする条件です。 これは、条件を定義するユーザー定義の述語関数オブジェクトによって提供されます。 述語は 1 つの引数を取り、`true` または `false` を返します。
...
`first` ランダム アクセス反復子の範囲内の最初の要素の位置に並べ替えられます。

`sortEnd` ランダム アクセス反復子の 1 つ位置過去のサブ範囲の最後の要素を並べ替えます。

`last` ランダム アクセス反復子、いずれかの過去の最後の要素範囲内の位置を部分的に並べ替えられます。
...
`first1` ソース範囲内の最初の要素の位置を示す入力反復子。

`last1` 1 つの後ろの位置、最後の要素、ソース範囲を示す入力反復子。

`first2` 並べ替えられたターゲット範囲の最初の要素の位置を示すランダム アクセス反復子。

`last2` 並べ替えられたターゲット範囲の過去の最後の要素いずれかの位置を示すランダム アクセス反復子。
...
`first` 双方向反復子の範囲内の最初の要素の位置をパーティション分割します。

`last` 双方向反復子の 1 つ位置過去の範囲の最後の要素を分割します。

`comp` 要素が分類される場合に満たされる条件を定義するユーザー定義の述語関数オブジェクトです。 述語は 1 つの引数を取り、**true** または **false** を返します。
...
`first` 条件を確認する範囲の先頭を示す入力反復子。

`last` 範囲の終了を示す入力反復子。

`dest1` 出力反復子を使用して条件の場合は true を返す要素のコピーを使用してテスト`_Pred`です。

`dest2` 出力反復子を使用して条件の場合は false を返す要素のコピーを使用してテスト`_Pred`です。

`_Pred` テストする条件です。 これは、テストする条件を定義するユーザー定義の述語関数オブジェクトによって提供されます。 述語は 1 つの引数を取り、`true` または `false` を返します。
...
`first` A`ForwardIterator`条件を確認する範囲の始まりを示すです。

`last` A`ForwardIterator`範囲の末尾を示すです。
...
`first` ヒープ内の最初の要素の位置を示すランダム アクセス反復子。

`last` ヒープの過去の最後の要素いずれかの位置を示すランダム アクセス反復子。
...
`first` 元の要素が削除される範囲内の最初の要素の位置を示す前方反復子。

`last` 過去の元となる要素が削除される範囲の最後の要素のいずれかの位置を示す前方反復子。
...
`first` 元の要素が削除される範囲内の最初の要素の位置を示す入力反復子。

`last` 過去の元となる要素が削除される範囲の最後の要素の 1 つの位置を示す入力反復子。

`result` 要素が削除されるターゲット範囲の最初の要素の位置を示す出力反復子。
...
`_Pred` 満たす必要のある単項述語は要素の値に置き換えられます。
...
`first` 元の要素が削除される範囲内の最初の要素の位置を指す前方反復子。

`last` 過去の元となる要素が削除される範囲の最後の要素の 1 つの位置を指す前方反復子。
...
`first` 要素が置き換えられる元の範囲内の最初の要素の位置を指す前方反復子。

`last` 過去の要素が置き換えられる元の範囲の最後の要素の 1 つの位置を指す前方反復子。

`_OldVal` 置き換えられる要素の古い値。

`_NewVal` 古い値を持つ要素に割り当てられる新しい値。
...
`first` 元となる要素が置き換えられる範囲内の最初の要素の位置を指し示す入力反復子。

`last` 1 つの後ろの位置、最後の要素範囲の要素が置き換えられる元となる指し示す入力反復子。

`result` 要素の変更のシーケンスのコピー先ターゲット範囲の最初の要素を指し示す出力反復子。
...
`result` 要素のコピー先ターゲット範囲の最初の要素の位置を指し示す出力反復子。

`_Pred` 満たす必要のある単項述語は要素の値に置き換えられます。

`val` 古い値を持つ述語を満たす要素に割り当てられる新しい値。
...
`last` 過去の要素が置き換えられる元の範囲の最後の要素の 1 つの位置を指す反復子。
...
`first` 要素の変えるされる範囲内の最初の要素の位置を指す双方向反復子。

`last` 1 つの後ろの位置、最後の要素の要素は変えるされる範囲を指す双方向反復子。
...
`first` 要素の変えるされるソース範囲の最初の要素の位置を指定する双方向反復子。

`last` 要素の変えるされるソース範囲内で過去の最後の要素いずれかの位置を指定する双方向反復子。
...
`first` 前方反復子の範囲の最初の要素の位置を示す回転します。

`middle` 範囲を要素と交換される範囲の最初の部分で範囲の 2 番目の部分内の最初の要素の位置を指す、境界を定義する前方反復子。

`Last` 前方反復いずれかの過去の最後の要素範囲内の位置、回転します。
...
_`Last`回転する前方反復子のいずれかの後ろの位置、最後の要素範囲のアドレスを指定します。

`result` ターゲット範囲内の最初の要素の位置を示す出力反復子。
...
`count` 検索対象のサブシーケンスのサイズ。

`val` 検索対象のシーケンス内の要素の値。
...
`first1` 結合され、2 つのソース範囲の差を表す単一の範囲に並べ替えられる 2 つの並べ替えられたソース範囲の最初の最初の要素の位置を示す入力反復子。

`last1` 入力反復子の位置を示すいずれかの過去の最後の要素の最初の 2 つの並べ替えられたソース範囲結合され、2 つのソース範囲の差を表す単一の範囲に並べ替えられます。

`first2` 入力反復子の最初の要素の位置の 2 つの連続する 2 番目の並べ替え済みソース範囲に結合され、2 つのソース範囲の差を表す単一の範囲に並べ替えられます。

`last2` 後ろの 1 つの位置を示す入力反復子の 2 つの連続する 2 番目の最後の要素には、ソース範囲に結合され、2 つのソース範囲の差を表す単一の範囲に並べ替えられるが並べ替えられます。

`result` 2 つのソース範囲の差を表す単一の並べ替えられた範囲に結合するのには 2 つのソース範囲がターゲット範囲内の最初の要素の位置を示す出力反復子。
...
`first1` 結合され、2 つのソース範囲の交差を表す単一の範囲に並べ替えられる 2 つの並べ替えられたソース範囲の最初の最初の要素の位置を示す入力反復子。

`last1` 入力反復子の位置を示すいずれかの過去の最後の要素の最初の 2 つの並べ替えられたソース範囲結合され、2 つのソース範囲の交差を表す単一の範囲に並べ替えられます。

`first2` 入力反復子の最初の要素の位置の 2 つの連続する 2 番目の並べ替え済みソース範囲に結合され、2 つのソース範囲の交差を表す単一の範囲に並べ替えられます。

`last2` 後ろの 1 つの位置を示す入力反復子の 2 つの連続する 2 番目の最後の要素には、ソース範囲に結合され、2 つのソース範囲の交差を表す単一の範囲に並べ替えられるが並べ替えられます。

**_** *結果*2 つのソース範囲ターゲット範囲の最初の要素の位置を示す出力反復子は、2 つのソースの交差を表す単一の並べ替えられた範囲に結合するのには範囲です。
...
`first1` 結合され、2 つのソース範囲の対称差を表す単一の範囲に並べ替えられる 2 つの並べ替えられたソース範囲の最初の最初の要素の位置を示す入力反復子。

`last1` 入力反復子の位置を示すいずれかの過去の最後の要素の最初の 2 つの並べ替えられたソース範囲結合され、2 つのソース範囲の対称差を表す単一の範囲に並べ替えられます。

`first2` 入力反復子の最初の要素の位置の 2 つの連続する 2 番目の並べ替え済みソース範囲に結合され、2 つのソース範囲の対称差を表す単一の範囲に並べ替えられます。

`last2` 後ろの 1 つの位置を示す入力反復子の 2 つの連続する 2 番目の最後の要素には、ソース範囲に結合され、2 つのソース範囲の対称差を表す単一の範囲に並べ替えられるが並べ替えられます。

**_** *結果*2 つのソース範囲ターゲット範囲の最初の要素の位置を示す出力反復子は、2 つの対称差を表す単一の並べ替えられた範囲に結合するのにはソースの範囲です。
...
`first1` 結合され、2 つのソース範囲の和集合を表す単一の範囲に並べ替えられる 2 つの並べ替えられたソース範囲の最初の最初の要素の位置を示す入力反復子。

`last1` 入力反復子の位置を示すいずれかの過去の最後の要素の最初の 2 つの並べ替えられたソース範囲結合され、2 つのソース範囲の和集合を表す単一の範囲に並べ替えられます。

`first2` 入力反復子の最初の要素の位置の 2 つの連続する 2 番目の並べ替え済みソース範囲に結合され、2 つのソース範囲の和集合を表す単一の範囲に並べ替えられます。

`last2` 後ろの 1 つの位置を示す入力反復子の 2 つの連続する 2 番目の最後の要素には、ソース範囲に結合され、2 つのソース範囲の和集合を表す単一の範囲に並べ替えられるが並べ替えられます。

**_** *結果*2 つのソース範囲ターゲット範囲の最初の要素の位置を示す出力反復子が次の 2 つのソース範囲の和集合を表す単一の並べ替えられた範囲に結合するのには。
...
`first` 反復子の範囲の最初の要素をシャッフル、包括的にします。 
          `RandomAccessIterator` および `ValueSwappable` の要件を満たしている必要があります。

`last` 反復子範囲の最後の要素をシャッフル、排他的なであります。 
          `RandomAccessIterator` および `ValueSwappable` の要件を満たしている必要があります。

`gen` 乱数ジェネレーターを`shuffle()`関数で操作に使用されます。 `UniformRandomNumberGenerator` の要件を満たしている必要があります。
...
`last` ランダム アクセス反復子の 1 つ位置過去の範囲の最後の要素を並べ替えます。

`comp` 順序の一連の要素によって満たされると、比較条件を定義するユーザー定義の述語関数オブジェクト。 この二項述語は、2 つの引数を取り、2 つの引数が順番どおりの場合は `true`、それ以外の場合は `false` を返します。 この比較子関数は、シーケンスからの要素のペアで厳密弱順序を強制する必要があります。 詳細については、「[アルゴリズム](../standard-library/algorithms.md)」を参照してください。
...
`first` ターゲット ヒープ内の最初の要素の位置を示すランダム アクセス反復子。

`last` 過去のコピー先のヒープの最後の要素のいずれかの位置を示すランダム アクセス反復子。
...
`_Pred` 要素が分類される場合に満たされる条件を定義するユーザー定義の述語関数オブジェクトです。 述語は 1 つの引数を取り、**true** または **false** を返します。
...
`first` 双方向反復子の範囲内の最初の要素の位置に並べ替えられます。

`last` 双方向反復子の 1 つ位置過去の範囲の最後の要素を並べ替えます。
...
`left` 最初の上書きでは、その内容が交換する最初のオブジェクト。 2 番目のオーバーライドでは、オブジェクトの最初の配列がそのコンテンツを交換させます。

`right` 最初の上書きでは、その内容を交換する 2 番目のオブジェクト。 2 番目のオーバーライドでは、オブジェクトの 2 番目の配列がそのコンテンツを交換させます。
...
`first1` 要素を持つが、交換するには、最初の範囲の最初の位置を指す前方反復子。

`last1` 1 つ後ろを交換する要素を持つは、最初の範囲の最後の位置を指す前方反復子。

`first2` 交換する要素は、2 番目の範囲の最初の位置を指す前方反復子。
...
`first1` 処理される最初のソース範囲内の最初の要素の位置を示す入力反復子。

`last1` 過去の最初のソース範囲の最後の要素のいずれかの位置を示す入力反復子で操作。

`first2` 処理される 2 番目のソース範囲内の最初の要素の位置を示す入力反復子。

`result` ターゲット範囲内の最初の要素の位置を示す出力反復子。

`_Func` ユーザー定義の単項関数の最初のソース範囲内の各要素に適用されるアルゴリズムの最初のバージョンで使用されるオブジェクトまたはユーザー定義 (UD) 二項関数オブジェクトはペアワイズ、順方向順序で適用されるアルゴリズムの 2 番目のバージョンで使用されます。、次の 2 つのソース範囲にします。
...
`first` 重複除去をスキャンする範囲内の最初の要素の位置を示す前方反復子。

`last` 1 つの後ろの位置、最後の要素の重複除去をスキャンする範囲を示す前方反復子。
...
`first` 前方反復子のソース範囲の最初の要素の位置を示すをコピーします。

`last` 前方反復子過去のソース範囲の最後の要素の 1 つの位置のアドレスをコピーします。

`result` 連続する重複してコピーを受け取るターゲット範囲の最初の要素の位置を示す出力反復子が削除されます。
...
`first` 検索対象の範囲内の最初の要素の位置。

`last` 1 つの後ろの位置最後の要素を検索する範囲。

`value` 返される反復子によってアドレス指定される要素の値によって超過する必要がある順序付けられた範囲の値。

---
title: "レコードセット: テーブルにアクセスするレコードセット クラスの宣言 (ODBC) | Microsoft Docs"
ms.custom: ""
ms.date: "11/04/2016"
ms.reviewer: ""
ms.suite: ""
ms.technology: 
  - "devlang-cpp"
ms.tgt_pltfrm: ""
ms.topic: "article"
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "ODBC レコードセット, 宣言 (テーブルのクラスを)"
  - "レコードセット, 宣言 (テーブルのクラスを)"
ms.assetid: 3fe286c2-3f3d-493d-9d8c-762310939d08
caps.latest.revision: 8
author: "mikeblome"
ms.author: "mblome"
manager: "ghogen"
caps.handback.revision: 8
---
# レコードセット: テーブルにアクセスするレコードセット クラスの宣言 (ODBC)
[!INCLUDE[vs2017banner](../../assembler/inline/includes/vs2017banner.md)]

このトピックの内容は、MFC ODBC クラスに該当します。  
  
 最も一般的なレコードセット クラスでは、単一のテーブルを開きます。  単一のテーブルのレコードセット クラスを宣言するには、クラスの追加の [MFC ODBC コンシューマー ウィザード](../../mfc/reference/adding-an-mfc-odbc-consumer.md) を使って、対応するレコードセットのフィールド データ メンバーを指定して各列を選択します。  
  
 レコードセットでは、以下の操作もできます。  
  
-   複数のテーブルの結合  
  
-   定義済みクエリの結果の保存  
  
## 参照  
 [レコードセット \(ODBC\)](../../data/odbc/recordset-odbc.md)   
 [レコードセット: レコードセットの生成と破棄 \(ODBC\)](../../data/odbc/recordset-creating-and-closing-recordsets-odbc.md)   
 [レコードセット: 定義済みクエリを利用したクラスの宣言 \(ODBC\)](../../data/odbc/recordset-declaring-a-class-for-a-predefined-query-odbc.md)   
 [レコードセット: 結合 \(ODBC\)](../Topic/Recordset:%20Performing%20a%20Join%20\(ODBC\).md)
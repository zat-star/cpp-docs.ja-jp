...
|AFX_WM_PROPERTY_CHANGED|プロパティ グリッド コントロールの所有者に送信 (`CMFCPropertyGridCtrl`)、ユーザーが選択されているプロパティの値を変更するときにします。|プロパティ リストのコントロール ID。|プロパティへのポインター (`CMFCProp``ertyGridProperty`) に変更します。|使用しません。|  

...
> [!IMPORTANT]
>  コードが Windows NT 4、Windows 2000、または Windows XP (SP1 以前) で実行する場合、Dll の完全パスを指定することを確認してください。 これらのオペレーティング システムでファイルが読み込まれると、最初に現在のディレクトリが検索されます。 ファイルのパスを指定していないと、目的のファイルでないファイルが読み込まれる場合があります。  

...
古いバージョンのコンパイラには、キーワードは不要`template`と`typename`everywhere、C++ 標準で必要なです。 いくつかの位置には、コンパイラが参照の最初のフェーズ中に依存している名前を解析する必要がある方法を区別するためにこれらのキーワードが必要です。 例:
...
この動作は、キーワードを排除することによっても確認できます`typename`関数テンプレート本体、初期化子、既定の引数、および noexcept 引数名の前にします。 例:
